...
window.Digest = (function () {
  ...
  function wireKeepTools(box, list) {
    const cp = box.querySelector("#ktCopy"); if (cp) cp.onclick = async () => {
      ...
      const html = `<table border="1" cellspacing="0" cellpadding="4"><tr><th>선례</th><th>처분</th><th>금액</th><th>연도</th><th>출처(감사원 · 순번)</th></tr>${rows}</table>`;
  ...
  async function openLawModal(p, date) {
    ...
    try {
      ...
      if (r && r.ok && r.text) {
        ...
        else if (et === "absent") msg = "법제처(법령·행정규칙·자치법규)에서 찾지 못했습니다. <b>정관·내규 등 비공개 규정</b>일 수 있어 원문(감사원·해당 기관)에서 확인하세요.";
  ...
  // ----- 감사원 원문(전문) -----
  const BAI_LIST = "https://www.bai.go.kr/bai/result/branch/list";
  ...
  function openBai(name) { try { navigator.clipboard.writeText(name || ""); } catch (e) {} window.open(BAI_LIST, "_blank"); toast("‘" + (name || "").slice(0, 22) + "…’ 제목을 복사했습니다 — 감사원 검색창에 붙여넣어 찾으세요"); }
  ...
  function caseHtml(c, opts) {
    ...
    return `
      ...
      <div class="dt-m">${dispBadges(c)}${(c.위반유형 || []).filter(v => !String(v).startsWith("(")).map(v => `<span class="vt">${esc(v)}</span>`).join("")}${(c.소주제 || []).map(v => `<span class="subtag" title="세부 주제">${esc(v)}</span>`).join("")}<span class="yr">· ${c.연도 || ""} · ${esc(c.기관계열 || "")}</span></div>
      ${flowStrip(c)}
      ...
      ${(c.형사연계 && c.형사연계.length) ? `<div class="crimebadge" title="감사원이 수사기관에 수사요청·의뢰하거나 고발(감사원법 §35)한 사건입니다. 주처분(징계·통보 등)과 별개로 부가될 수 있어 별도로 식별합니다. 처분 수위·범죄 성립 판단과는 무관하며 원문을 확인하세요.">⚖️ 형사 연계 · ${c.형사연계.map(esc).join(" · ")}</div>` : ""}
      ${c.요지 ? `<div class="dt-sec prime"><h4>요지 〈감사원 공개문 인용·발췌〉</h4><div class="body">${fmtBody(c.요지)}</div></div>` : ""}
      ...
      <div class="dt-sec facts"><h4>출처 · 원문</h4>
        <div class="body" style="font-size:12px;color:var(--muted)">${esc(c.감사명 || "")}<br>공개일 ${c.공개일 || ""} · 감사종류 ${esc(c.감사종류 || "")} · 순번 ${c.srno}${isCsd ? " · (CSD 원문·검색전용)" : ""}</div>
        <div class="docbtns"><button class="docbtn js-bai">🔎 감사원에서 찾기</button><button class="docbtn js-memo">📝 메모에 담기</button><button class="copybtn js-cite" style="margin-top:0">📋 인용 복사</button></div>
      ...
      ${(typeof c.비지적 === "string" && c.비지적.trim().length > 8) ? `<div class="dt-sec nonflag"><h4>✅ 비지적 · 참고 〈공개문 발췌〉</h4><div class="body">${fmtBody(c.비지적)}</div></div>` : ""}
  ...
  function wireCase(root, c) {
    ...
    const bb = root.querySelector(".js-bai"); if (bb) bb.onclick = () => openBai(c.감사명);
    const mb = root.querySelector(".js-memo"); if (mb) mb.onclick = () => { try { window.Memo.addCase(c); } catch (e) {} };
    const cp = root.querySelector(".js-cite"); if (cp) cp.onclick = () => { const t = `${c.제목} (감사원 ${c.감사명}, 순번 ${c.srno}${(c.근거페이지 || []).length ? ", " + c.근거페이지.join("·") : ""})`; try { navigator.clipboard.writeText(t); } catch (e) {} cp.textContent = "✓ 복사됨"; setTimeout(() => cp.textContent = "📋 인용 복사", 1500); };
  ...
  // ----- 분야 그리드 + 위반유형 -----
  const FLDORDER = ["계약·조달", "재정·세무·회계", "보조금·기금", "인사·채용", "공유재산·물품", "인허가·개발", "환경·에너지", "복지·보건", "교통·물류", "도시·주택·건축", "농림·축산·수산", "산업·일자리", "안전·재난", "교육·평생학습", "정보·AI·디지털", "문화·관광·체육", "여성·가족·아동", "공공시설 관리·운영", "일반행정"];   // 19분야 재설계(거버넌스6·기능11·시설·일반)
  ...
  function qMatch(c, p) { const hay = hayOf(c); return p.ph.every(t => hit(c, hay, t)) && p.inc.every(t => hit(c, hay, t)) && !p.exc.some(t => hay.includes(t)); }
  ...
  function init() {
    ...
    bdg.innerHTML = `감사원 사례 <b>${fmt(NZ.length)}</b> · ${fmt(nzDocs)}문서` + (csd ? ` <span style="opacity:.6">+ CSD ${fmt(csd)}</span>` : "");
    const lastDt = NZ.reduce((m, c) => (c.공개일 || "") > m ? c.공개일 : m, "");   // 데이터 신선도 고지
    if (lastDt) bdg.title = `감사원 공개 감사결과 기준 — 최신 공개일 ${lastDt} 까지 수록`;
    buildFieldGrid();
