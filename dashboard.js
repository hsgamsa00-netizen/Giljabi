...
window.Dashboard = (function () {
  ...
  const FLD = ["계약·조달", "재정·세무·회계", "보조금·기금", "인사·채용", "공유재산·물품", "인허가·개발", "환경·에너지", "복지·보건", "교통·물류", "도시·주택·건축", "농림·축산·수산", "산업·일자리", "안전·재난", "교육·평생학습", "정보·AI·디지털", "문화·관광·체육", "여성·가족·아동", "공공시설 관리·운영", "일반행정"];   // 19분야 재설계
  const YV = ["검사·감독", "제도·후속", "재정·세입", "계약·조달", "인허가·개발", "인사·복무", "청렴·행위", "건설·시설", "문서", "보조금·연구비", "손해·변상"];   // 위반유형 통제어휘 11종 전체(빈도순·기타 제외)
  const DORD = [["주의요구", "ju"], ["시정요구", "si"], ["개선요구", "gae"], ["권고", "gw"], ["통보", "tong"], ["징계·문책요구", "mun"], ["변상판정(유책)", "bs"], ["변상판정(일부)", "bs"], ["고발", "go"], ["수사의뢰", "go"]];
  const GYE3 = ["중앙", "공공기관", "지자체"];
  ...
  function init() {
    ...
    const yrTip = "감사원이 최근 공개한 공개문을 중심으로 수집한 자료로, 그 이전 연도는 수록 건수가 적습니다. 표본이 작은 연도는 개별 사례 단위로 참고하십시오.";
